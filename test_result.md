...
agent_communication:
  ...
  - agent: "main"
    message: "🔧 EAS BUILD FIX (2026-05-04): Resolved EAS Android build failure root cause. (1) Fixed splash-screen filename mismatch in app.json: `./assets/images/splash-icon.png` → `./assets/images/splash-image.png` (the splash-icon.png file did not exist, causing prebuild to fail silently before expo-build-properties could write minSdkVersion=26). (2) Verified `expo-build-properties@1.0.10` plugin correctly writes `android.minSdkVersion=26` to `gradle.properties` after successful prebuild — local prebuild test passed. (3) Added `frontend/android/` and `frontend/ios/` to `.gitignore` so EAS always runs fresh prebuild. The minSdkVersion 24 vs 26 conflict for `androidx.health.connect:connect-client:1.1.0-alpha11` should now be resolved on next EAS build."
