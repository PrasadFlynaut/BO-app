...
export default function FeedScreen() {
  ...
  const loadFeed = async () => {
    try { const { data } = await api.get('/feed'); setPosts(data.posts); } catch (e) { console.error(e); }
  };
  ...
  const addComment = async (postId: string) => {
    ...
    try {
      await api.post(`/feed/${postId}/comment`, { content: newComment });
      setNewComment('');
    ...
    <SafeAreaView style={styles.safe}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <View style={styles.newPostCard}>
          ...
          <TouchableOpacity testID="feed-post-submit" onPress={createPost} style={styles.sendBtn} activeOpacity={0.7}>
            ...
            <View style={styles.postCard}>
              ...
              <View style={styles.postActions}>
                <TouchableOpacity testID={`like-${item.id}`} style={styles.actionBtn} onPress={() => toggleLike(item.id)}>
                  <Ionicons name={item.liked_by_me ? 'heart' : 'heart-outline'} size={20} color={item.liked_by_me ? Colors.danger : Colors.textMuted} />
              ...
              {showComments === item.id && (
                <View style={styles.commentsSection}>
                  ...
                  <View style={styles.commentInputRow}>
                    ...
                    <TouchableOpacity onPress={() => addComment(item.id)} style={styles.commentSend}>
...
const styles = StyleSheet.create({
  ...
  sendBtn: { backgroundColor: Colors.secondary, borderRadius: Radius.sm, padding: Spacing.sm },
  ...
  postAvatarText: { color: Colors.secondary, fontWeight: '700', fontSize: 13 },
  ...
  commentUser: { color: Colors.primary, fontWeight: '600', fontSize: FontSize.caption },
  ...
  commentInput: { flex: 1, backgroundColor: Colors.bgBase, borderRadius: Radius.sm, paddingVertical: 8, paddingHorizontal: Spacing.sm, color: Colors.textPrimary, fontSize: FontSize.caption },
