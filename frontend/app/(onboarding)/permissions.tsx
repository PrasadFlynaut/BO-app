...
const PERMISSIONS = [
  { id: 'push_notifications', label: 'Push Notifications', desc: 'Get reminders for meals, water, and goals', icon: 'notifications-outline' as const, color: Colors.nutritionOrange },
  { id: 'gallery_access', label: 'Photo Gallery', desc: 'Upload photos for recipes and posts', icon: 'images-outline' as const, color: Colors.waterBlue },
  { id: 'location_sharing', label: 'Location', desc: 'Discover nearby restaurants', icon: 'location-outline' as const, color: Colors.green },
  { id: 'data_personalization_consent', label: 'Data Personalization', desc: 'Use your data to personalize recommendations', icon: 'analytics-outline' as const, color: Colors.fitnessPurple },
];
      ...
      <ScrollView contentContainerStyle={st.scroll} showsVerticalScrollIndicator={false}>
        <Animated.View entering={FadeInDown.duration(500)}>
          <LinearGradient colors={[Colors.nutritionOrange + '15', 'transparent']} style={st.stepBadge}>
            <Text style={st.step}>Step 7 of 8</Text>
          </LinearGradient>
        ...
        {PERMISSIONS.map((p, i) => (
          <Animated.View key={p.id} entering={FadeInDown.delay(i * 80).duration(400)}>
            <TouchableOpacity style={[st.permRow, enabled[p.id] && { borderColor: p.color, backgroundColor: p.color + '08' }]} onPress={() => toggle(p.id)} activeOpacity={0.7}>
              ...
              <View style={[st.toggleSwitch, enabled[p.id] && { backgroundColor: p.color }]}>
                <Ionicons name={enabled[p.id] ? 'checkmark' : 'close'} size={14} color={enabled[p.id] ? '#FFF' : Colors.textTertiary} />
              </View>
            </TouchableOpacity>
          </Animated.View>
        ))}
        ...
        <Animated.View entering={FadeInDown.delay(500).duration(500)}>
          <TouchableOpacity testID="permissions-continue" onPress={handleContinue} disabled={!hipaaAccepted || !privacyAccepted} activeOpacity={0.9}>
            <LinearGradient colors={(!hipaaAccepted || !privacyAccepted) ? [Colors.textTertiary, Colors.textTertiary] : [Colors.nutritionOrange, '#E88A10']} style={[st.button, Shadow.md]}>
