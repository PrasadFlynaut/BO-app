    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.scroll}>
          ...
          {DIETS.map(d => {
            ...
            return (
              <TouchableOpacity key={d.id} testID={`diet-${d.id}`} style={[styles.chip, active && styles.chipActive]} onPress={() => toggleDiet(d.id)} activeOpacity={0.7}>
                <Ionicons name={d.icon} size={20} color={active ? Colors.secondary : Colors.textMuted} />
        ...
        <TouchableOpacity
          ...
          style={styles.button}
          onPress={() => router.push({ pathname: '/(onboarding)/health', params: { goals: params.goals as string, diets: JSON.stringify(diets), allergies: JSON.stringify(allergies) } })}
          activeOpacity={0.8}
        >
...
const styles = StyleSheet.create({
  ...
  chipActive: { borderColor: Colors.secondary, backgroundColor: 'rgba(219,255,2,0.08)' },
